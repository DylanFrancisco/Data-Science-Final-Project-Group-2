...
split <- sample.split(hhadata2015.seperated$Total.HHA.Medicare.Payment.Amount, SplitRatio = 0.8)
train_data <- filter(hhadata2015.seperated, split == TRUE)
test_data <- filter(hhadata2015.seperated, split == FALSE)

# Create a linear model
lm_model <- lm(Average.HHA.Medicare.Payment.Amount ~ Service_Severity_Level + Functional_Severity_Level + Clinical_Severity_Level, data = train_data)

# Make predictions on the test set
predictions <- predict(lm_model, newdata = test_data)

# Calculate the model performance metrics (
mse <- mean((test_data$Average.HHA.Medicare.Payment.Amount - predictions)^2)
rmse <- sqrt(mse)

# View the performance metrics
print(paste("Root Mean Squared Error (RMSE):", rmse))

test_data$residuals <- test_data$Average.HHA.Medicare.Payment.Amount - predictions

plot_data1 <- ggplot(test_data, aes(x = Service_Severity_Level, y = residuals)) +
  geom_point() +
  ...
  theme_minimal()
plot_data1
plot_data2 <- ggplot(test_data, aes(x = Functional_Severity_Level, y = residuals)) +
  geom_point() +
  geom_hline(yintercept = 0, color = "red", linetype = "dashed") +
  ...
  theme_minimal()
plot_data2
plot_data3 <- ggplot(test_data, aes(x = Clinical_Severity_Level, y = residuals)) +
  geom_point() +
  geom_hline(yintercept = 0, color = "red", linetype = "dashed") +
...
combined_data <- bind_rows(
  data.frame(variable = "Service_Severity_Level", test_data),
  data.frame(variable = "Functional_Severity_Level", test_data),
  data.frame(variable = "Clinical_Severity_Level", test_data)
)

# Create the overlapped plot
ggplot(test_data, aes(x = as.numeric(Service_Severity_Level), y = residuals, color = "Service_Severity_Level")) +
  geom_point() +
  geom_hline(yintercept = 0, color = "red", linetype = "dashed") +
  ...
  theme_minimal() +
  geom_point(data = test_data, aes(x = as.numeric(Functional_Severity_Level), y = residuals, color = "Functional_Severity_Level")) +
  geom_point(data = test_data, aes(x = as.numeric(Clinical_Severity_Level), y = residuals, color = "Clinical_Severity_Level")) +
...
#plot_data <- plot_data + facet_wrap(~Clinical_Severity_Level, ncol = 1)
